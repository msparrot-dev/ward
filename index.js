...
for (const module of slashCommands) {
        ...
        for (const commandFile of commandFiles) {
                const command = require(
                        `./public/slash/${module}/${commandFile}`,
                );
...
async function addRole(userID) {
        try {
                ...
                member.roles
                        ...
                        .catch(() => {
                                logger.error(
                                        `Failed to add role to user ${member.user.tag}! (Maybe verified role is above bot role?)`,
                                );
                                return;
                        })
                        .then(() => {
                                logger.info(
                                        `Added verified role to user ${member.user.tag}.`,
                                );
                        });
        } catch (e) {
                console.log(e);
        ...
        if (removeRole) {
                try {
                        const guild = await client.guilds.fetch(
                                ...
                                removeRoleId = await guild.roles.fetch(
                                        config.Discord.removeRoleId,
                                ),
                                member = await guild.members.fetch(userID);
                        ...
                        member.roles
                                ...
                                .catch(() => {
                                        logger.error(
                                                `Failed to remove role from user ${member.user.tag}! (Maybe role is above bot role?)`,
                                        );
                                        return;
                                })
                                .then(() => {
                                        logger.info(
                                                `Removed role from user ${member.user.tag}.`,
                                        );
                                });
                } catch (e) {
                        logger.error(
                                `Failed to remove role from user ${userID}!`,
                        );
                }
        } else {
...
app.get("/verify/:verifyId?", (req, res) => {
        if (!req.params.verifyId)
                return res.sendFile(
                        path.join(__dirname, "/html/invalidLink.html"),
                );
        if (!pool.isValidLink(req.params.verifyId))
                return res.sendFile(
                        path.join(__dirname, "/html/invalidLink.html"),
                );
...
app.post("/verify/:verifyId?", async (req, res) => {
        ...
        if (!response.data.success)
                return res.sendFile(
                        path.join(__dirname, "/html/invalidCaptcha.html"),
                );
        if (!pool.isValidLink(req.params.verifyId))
                return res.sendFile(
                        path.join(__dirname, "/html/invalidLink.html"),
